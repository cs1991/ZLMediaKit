...
void FFmpegSource::play(
    const string &ffmpeg_cmd_key, const string &src_url, const string &dst_url, int timeout_ms, const onPlay &cb) {
    ...
    if (is_local_ip(_media_info._host)) {
        ...
        findAsync(timeout_ms, [cb, weakSelf, timeout_ms](const MediaSource::Ptr &src) {
            ...
            if (!strongSelf->_process.wait(false)) {
                // ffmpeg进程已经退出
                cb(SockException(
                    Err_other, StrPrinter << "ffmpeg已经退出,exit code = " << strongSelf->_process.exit_code()));
                return;
            }
...
void FFmpegSource::findAsync(int maxWaitMS, const function<void(const MediaSource::Ptr &src)> &cb) {
    auto src = MediaSource::find(_media_info._schema, _media_info._vhost, _media_info._app, _media_info._streamid);
    if (src || !maxWaitMS) {
        cb(src);
        return;
    }
    ...
    auto onRegist = [listener_tag, weakSelf, cb, onRegistTimeout](BroadcastMediaChangedArgs) {
        ...
        if (!bRegist || sender.getSchema() != strongSelf->_media_info._schema
            || sender.getVhost() != strongSelf->_media_info._vhost || sender.getApp() != strongSelf->_media_info._app
        ...
        [weakSelf, timeout_ms]() {
            ...
            if (is_local_ip(strongSelf->_media_info._host)) {
                ...
                strongSelf->findAsync(0, [&](const MediaSource::Ptr &src) {
                    ...
                    if (!src) {
                        ...
                        if (strongSelf->_replay_ticker.elapsedTime() > 20 * 1000) {
                            ...
                            strongSelf->play(
                                strongSelf->_ffmpeg_cmd_key, strongSelf->_src_url, strongSelf->_dst_url, timeout_ms,
                                [](const SockException &) {});
                        }
                    }
                });
                ...
                if (!strongSelf->_process.wait(false)) {
                    ...
                    strongSelf->play(
                        strongSelf->_ffmpeg_cmd_key, strongSelf->_src_url, strongSelf->_dst_url, timeout_ms,
                        [weakSelf](const SockException &ex) {
                            if (!ex) {
                                //没有错误
                                return;
                            }
...
void FFmpegSnap::makeSnap(
    ...
    WorkThreadPool::Instance().getPoller()->async([timeout_sec, play_url, save_path, cb, ticker]() {
        ...
        //定时器延时应该减去后台任务启动的延时
        auto delayTask = EventPollerPool::Instance().getPoller()->doDelayTask(
            (uint64_t)(timeout_sec * 1000 - elapsed_ms), [process, cb]() {
                if (process->wait(false)) {
                    // FFmpeg进程还在运行，超时就关闭它
                    process->kill(2000);
                }
