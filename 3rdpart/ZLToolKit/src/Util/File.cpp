...
long File::getFileLastModify(const char *filename) {
    int result;
#ifdef WIN32
    struct _stat buf;
...
void File::delete_timeout_file(const char *path, long timeout) {
    DIR *dir;
    dirent *dir_info;
    char file_path[PATH_MAX];
    if (is_file(path)) {
        long fileTime = getFileLastModify(path);
        if (fileTime > 0) {
            time_t t;
            tzset(); /*tzset()*/
            t = time(NULL);
            //ErrorL << "当前秒数：" << t << "     file秒数:" << fileTime; 
            if ((t - fileTime) > timeout) //秒数
            {
                remove(path);
            }
        }
    ...
    if (is_dir(path)) {
        if ((dir = opendir(path)) != NULL) {
            while ((dir_info = readdir(dir)) != NULL) {
                ...
                get_file_path(path, dir_info->d_name, file_path);
                delete_timeout_file(file_path,timeout);
            }
        }
        bool isempty = true;
        if ((dir = opendir(path)) != NULL) {
